<%-- 
-- File Name: changePasswordUser
-- Description: This page displays Change Password Page for User self
-- Author(s): CTE. 
-- Date: 01/02/2016 
-- MOD HISTORY
-- DATE           	USER				COMMENTS
...
<head>
	...
	<script type="text/javascript">
		...
		function editSubmit() {		
			...
			$.ajax({
				url: "${request.getContextPath()}/mkUsers/getUserDetailsUser",
				async : false,
				data : parameter,
		...
		function fnOldPassword() {
			$("#blankerrormsg").html("");
			if($('#name').val()==""){
				$("#blankerrormsg").html("Username Field Should Not Be Blank!").show(); 
				$('#name').focus();
				return false;
			}else if($('#oldPwd').val()==""){
				...
				$('#oldPwd').focus();
				return false;
			}else if(($('#oldPwd').val()).length < 7){
				$("#blankerrormsg").html("Old Password Field Should Not Be Less Than 8 Characters!").show(); 
				$('#oldPwd').focus();
				return false;
			}else {
				return true;
			...
			var result = confirm("Please re-enter the password")
		 	if(result){	 	
				...
				if($('#name').val()==""){
					...
					$("#blankerrormsg").html("Confirm Password Field Should Not Be Equal to Old Password!").show();
					$('#newPwdConf').focus();
					return false;	
				} else {
					$("#newPwd").hide();
		...
		function savePassword() {
			...
			if($('#name').val()==""){
				...
				$("#blankerrormsg").html("New Password Field Should Not Be Equal to Old Password!").show();
				$('#newPwdConf').focus();
				return false;	
		...
		function clearAllFields(){
		 ...
		 function returnToMainPage(){
			 ...
			 if(result){
			 	document.getElementById("changePasswordUserForm").action ="${request.getContextPath()}/company/mainModule";
...
<body>
<form name="changePasswordUserForm" id="changePasswordUserForm">
				...
				<div class="modal-body">
					<table width="100%" id="editPassword" class="table">
						...
						<tbody id="prevAdvNoTbdId">
							<g:each in="${userListForUser}" status="i" var="userInst">
							...
							<div class="col-md-12">
								...
								<div class="row tab-form">
									<div class="col-md-6 col-sm-6 col-xs-6">
										...
										<div class="col-sm-8 col-md-8 col-xs-8">
											<g:passwordField name="oldPwd" id="oldPwd" class="form-control" value="" tabindex="2" onblur="fnOldPassword();"/>
										</div>
									</div>
								</div>
								<div class="row tab-form">
									<div class="col-md-6 col-sm-6 col-xs-6">
										<label for="concept" class="col-sm-4 control-label"><g:message
												code="mtlItemLocationsForm.description.label"
												default="New Password"  nowrap="nowrap"/></label>
										<div class="col-sm-8 col-md-8 col-xs-8">
