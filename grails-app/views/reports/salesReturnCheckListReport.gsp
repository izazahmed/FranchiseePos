<%-- 
     -- File Name: SalesReturnChkListReport
     -- Description: it is used to get the SalesReturnChkListReport
     -- Author(s): CTE 
     -- Date: 17/05/2016 
     -- MOD HISTORY
     -- DATE           USER             COMMENTS
...
$(document).ready(function(){	
	...
	$(".clickOnSubCode td:not(.action)").click(function () {
		subCodeVal =$(this).closest('tr').attr('id');
		$(".highlight").removeClass('highlight');
		$(this).closest('tr').addClass('highlight');
	});
...
function displayTextOnSubCode(){
 	$(".clickOnSubCode").removeClass('highlight');
...
function modifyCustomerFields(val){
  ...
  function validateSalesReturnReport(format,extension) { 		 
  	var flag = "";
  	flag = validate();
  	if(!flag){
  		return false;
  	}else{		
  		document.getElementById('salesRetChkListReportForm').format1.value = format;
		document.getElementById('salesRetChkListReportForm').extension.value = extension;
		
  		document.getElementById("salesRetChkListReportForm").action ="createSalesRetChkListReport";
        document.getElementById("salesRetChkListReportForm").submit();
  	}  	  	     	
  }
 function validate(){
 	var flag = "false";
				...
				<div class="dashboard_detail_bg">
					<div class="col-md-12">
						<div id="create-empMst" class="content scaffold-create">
							...
							<g:form name="salesRetChkListReportForm" id="salesRetChkListReportForm">
											...
											<div class="panel panel-default">
												<table class="table">
													<thead>
														...
														<tr>
															<td>End Date</td>
															<td colspan="2"><g:textField class="form-control endDateStr" placeholder="End Date" name="toDate" /></td>
														</tr>
														<tr>
															<td>Category</td>
															<td>
															...
															<td>
																<div class="select-style2">
																	<select id="location" name="location">
																		<g:each in="${branchResult}" status="i"
																			var="branchResultData">
																			<option value="${branchResultData.key}">
																				${branchResultData.value}
																			</option>
																		</g:each>
																	</select>
																</div>
														...
														<tr>
															<td colspan="2"><b>Sub Code</b></td>
														</tr>
														<tr>
															<td colspan="2"><g:radio name="subCodeSel" id="subCodeSel" value="" checked="ckecked" onchange="modifySubCodeFields('all');" />ALL</td>
														</tr>
														<tr>
															<td><g:radio name="subCodeSel" value=""
																	onchange="modifySubCodeFields('specific');" />Specific</td>
															<td>
																<div class="textbtn">
																	<g:textField class="form-control" size="17"
																		name="subCode" id="subCode" />
																	<button id="butIdSubCode" type="button"
																		class="active_btn"
																		onclick="getSubCodeDataBasedOnCat();">...</button>
																</div>
															</td>
														</tr>
														<tr>
														...
														<tr>
															<td colspan="2"><g:radio name="custSel" value=""
																	checked="ckecked"
																	onchange="modifyCustomerFields('all');" />ALL</td>
														</tr>
														<tr>
															<td><g:radio name="custSel" id="custSel" value=""
																	onchange="modifyCustomerFields('specific');" />Specific</td>
															<td>
																<div class="textbtn">
																	<g:textField class="form-control" size="17"
																		name="custName" />
																	<button id="butIdCust" type="button"
																		class="active_btn" data-toggle="modal"
																		data-target="#custModal">...</button>
																</div>
															</td>
														</tr>
												</table>											
								...
								<div class="row">
									<div class="col-md-8">
										<div class="pull-right">
											<g:submitButton name="Submit" value="View" class="active_btn" onclick="return validateSalesReturnReport('view','view')"></g:submitButton>
											<g:submitButton name="Submit" value="PDF" class="active_btn" onclick="return validateSalesReturnReport('pdf','pdf')"></g:submitButton>
											<g:submitButton name="Submit" value="CSV" class="active_btn" onclick="return validateSalesReturnReport('csv','csv')"></g:submitButton>
										</div>
								</div>
							</div>							
						</div>						
										...
										<table class="table">
											...
											<tbody id="catTblBdId">
												<g:each in="${descList}" var="descListInst" status="i">
													<tr class="${(i % 2) == 0 ? 'even' : 'odd'} clickOnCat"
														id="${descListInst?.vc_reason_desc},${descListInst?.vc_reason_code}">
														<td name="popDesc" id="popDesc">
															${descListInst?.vc_reason_desc}
														</td>
													</tr>
												</g:each>
											</tbody>
											...
											<table class="table">
												...
												<tbody>
													<g:each in="${itemList}" var="itemListInst" status="i">
														<tr
															class="${(i % 2) == 0 ? 'even' : 'odd'} editClickOnItem"
															id="${itemListInst?.VC_SALES_ID}">
															<td name="popItemId" id="popItemId">
																${itemListInst?.VC_SALES_ID}
															</td>
															<td name="popItemName" id="popItemName">
																${itemListInst?.EMP_NAME}
															</td>
														</tr>
													</g:each>
												</tbody>
						...
						<div class="modal fade" id="subCodeModal" role="dialog">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal">&times;</button>
										<h4 class="modal-title">SUBCODE List</h4>
									</div>
									<div class="modal-body">
										<table width="100%" id="subCodeTblId" class="table">
											<thead>
												<tr>
													<th>SubCode Name</th>
												</tr>
											...
											<tbody>
												<g:each in="${subcodeList}" status="i" var="subcodeListInst">
													<tr class="${(i % 2) == 0 ? 'even' : 'odd'} clickOnSubCode"
														id="${subcodeListInst?.VC_CUST_FNAME}">
														<td name="popSubCode" id="popSubCode">
															${subcodeListInst?.VC_CUST_FNAME}
														</td>
													</tr>
												</g:each>
											</tbody>
									...
									<div class="modal-footer">
										<button type="button" class="active_btn" onclick="displayTextOnSubCode();" data-dismiss="modal">OK</button>
										<button type="button" class="active_btn" data-dismiss="modal">Cancel</button>
									</div>
								</div>
							</div>
								...
								<div class="modal-content">
									...
									<div class="modal-body">
										<table width="100%" id="custTblId" class="table">
											...
											<tbody>
												<g:each in="${customerList}" status="i"
													var="customerListInst">
													<tr class="${(i % 2) == 0 ? 'even' : 'odd'} clickOnCust"
														id="${customerListInst?.VC_CUST_FNAME}">
														<td name="popCustName" id="popCustName">
															${customerListInst?.VC_CUST_FNAME}
														</td>
													</tr>
												</g:each>
											</tbody>
